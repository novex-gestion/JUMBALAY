...
function descontarStockPedido(string $id, string &$motivo): bool {
  ...
  foreach (($pedido['items'] ?? []) as $item) {
    ...
    if ($productoId === '' || $cantidad === false || $cantidad < 1) {
      $motivo = 'La orden tiene un producto o una cantidad inválida.'; return false;
    }
    $cantidades[$productoId] = ($cantidades[$productoId] ?? 0) + $cantidad;
  }
  if (!$cantidades) { $motivo = 'La orden no tiene productos.'; return false; }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  else {$id=(string)($_POST['id']??'');$accion=(string)($_POST['accion']??'guardar');if($accion==='descontar_stock'){$motivo='No se pudo actualizar el stock.';if(descontarStockPedido($id,$motivo))$aviso='Stock descontado. El panel de productos y la web ya muestran las cantidades nuevas.';else $error=$motivo;}elseif($accion==='confirmar_pago'){if(confirmarPago($id))$aviso='Pago confirmado.';else$error='No se pudo confirmar el pago.';}elseif($accion==='archivar'||$accion==='restaurar'){if(cambiarArchivado($id,$accion==='archivar'))$aviso=$accion==='archivar'?'Pedido archivado.':'Pedido reactivado.';else$error='No se pudo cambiar el archivo del pedido.';}else{$estado=(string)($_POST['estado']??'');$notas=mb_substr(trim((string)($_POST['notas']??'')),0,500);if(!$id||!isset(ESTADOS[$estado]))$error='No se pudo actualizar el pedido.';elseif(guardarPedido($id,$estado,$notas))$aviso=$estado==='cancelado'?'Pedido cancelado y archivado.':'Pedido actualizado.';else$error='No se pudo guardar el cambio.';}}
}
$vista=(($_GET['vista']??'')==='archivadas')?'archivadas':'activas';$todos=leerPedidos();$stockDescontado=catalogo()['stock_descontado_pedidos']??[];$estaArchivado=fn($p)=>(bool)($p['archivado']??false)||estadoPedido($p)==='cancelado';$pedidos=array_values(array_filter($todos,fn($p)=>$vista==='archivadas'?$estaArchivado($p):!$estaArchivado($p)));$conteo=[];foreach($pedidos as $p){$s=estadoPedido($p);$conteo[$s]=($conteo[$s]??0)+1;}$activas=count(array_filter($todos,fn($p)=>!$estaArchivado($p)));$archivadas=count($todos)-$activas;
?><!doctype html>
